...
st.set_page_config(page_title="My Habits",page_icon="favicon-32x32.png",layout="wide")
...
def Home():
    ...
    total_investment = float(pd.Series(df['Investment']).sum())
    investment_mode = float(pd.Series(df['Investment']).mode())
    investment_mean = float(pd.Series(df['Investment']).mean())
    investment_median= float(pd.Series(df['Investment']).median()) 
    rating = float(pd.Series(df['Rating']).sum())
